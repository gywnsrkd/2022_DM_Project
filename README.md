...
*  인구당 면적 비율이 높을수록 쓰레기 배출량이 증가한다<br>
*  도소매 종사자 비율이 높을수록 쓰레기 배출량이 올라간다. 


#### 가설과 다르게 판단된 내용

* 외국인 비율이 낮을수록/한국인 비율이 높을수록 쓰레기 배출량이 올라간다.<br>

>> 가설설정으로는 특정 지역의 관광지에 외국인이 많이 와서 쓰레기를 버리기떄문에 배출량이 올라간다고 생각했지만
우리 데이터는 각 자치구의 외국인 거주자이기 때문에 가설설정에서 세운 유동인구의 개념이 포함되지 않는다고 판단.
그래서 가설과 반대로 결과가 나왔다고 본다. 

* 석유 사용량 비율이 낮을수록 쓰레기 배출량이 올라간다.

>> 석유사용량이 많으면 그 지역에 사람이 많을것이라는 가정을 세웠지만 석유사용량은 그 지역의 인구 밀도랑은 
상관없는것으로 판단합니다. 주유를 할때 주변에 있는 주유소가 아닌 다른 싼 지역의 주유소에서 주유를 하면
그 주유의 배출량은 그 지역의 사용량으로 계산되어서 그 구의 인구밀도로 파악하기는 힘들다. 
이 또한 유동인구의 특성을 반영한 데이터가 아니라서 가설과 다르다고 판단했습니다.
...
#### 기대효과
* 가정생활폐기물 양에 영향을 주는 요인을 분석함으로써 쓰레기 증가에 원인을 파악하고 일을 줄이는 방향의 제도 마련 가능
* 가정생활폐기물 양 예측으로 처리 인력, 예산 분배에 도움

#### 한계점
* 데이터 개수가 부족해 자치구별 특징의 인사이트를 도출하기에는 한계가 있다.
* y 데이터의 변동이 작아서 x변수를 다르게 해도 비슷한 결과를 도출할수 있는 오류가능성 존재
